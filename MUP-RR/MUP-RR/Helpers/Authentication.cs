...
namespace MUP_RR.Helpers
{
    public class Authentication
    {
            ...
            try
            {
                ...
                if (session.IUPI == Guid.Empty)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(shibboleth.Email) || env.IsLocalhost(context))
                    {
                        ...
                        int userID = GetUserId(iupi, db);
                        session.UserID = userID;
                        session.Roles = GetRolesForUser(userID, db);
            ...
            DbModels.muprrdevContext db)
        {
            ...
            if (db.Users.Any(a => a.Id == personID && a.Active && a.IsAdmin))
                roles.Add(RoleAuthorization.Roles.admin);

            //verifica se é utilizador registado
            if (db.Users.Any(p => p.Id == personID))
