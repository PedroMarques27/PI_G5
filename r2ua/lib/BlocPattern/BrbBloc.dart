...
class BuildingBloc {
  ...
  Future<Building> getBuildingById(int id) async{
    var uri =
      Uri.https(BASE_URL, ("/api/Buildings/"+id.toString()));
    final response = await http.get(uri, 
      headers: {
        HttpHeaders.authorizationHeader: "Bearer $token",
        HttpHeaders.contentTypeHeader: "application/json",
...
class UsersBloc {
  StreamController<User> usersStreamController = StreamController<User>.broadcast();
...
class ClassroomsBloc {
  StreamController<List<Classroom>> classroomStreamController = StreamController<List<Classroom>>.broadcast();
  Stream get getClassrooms => classroomStreamController.stream;

  void update(List<Classroom> b) {
    classroomStreamController.sink.add(b); // add whatever data we want into the Sink
  }

  void dispose() {
    classroomStreamController.close(); // close our StreamController to emory leak
  }
  ...
  Future<List<Classroom>> getData(String url) async{
    ...
    Iterable l = json.decode(response.body)['data'];
    List<Classroom> classroom = List<Classroom>.from(l.map((model)=> Classroom.fromJson(model)));
  ...
  Future<Classroom> getClassroomById(int id) async{
    var uri =
      Uri.https(BASE_URL, "/api/Classrooms/"+id.toString());
    final response = await http.get(uri, 
      headers: {
        HttpHeaders.authorizationHeader: "Bearer $token",
        HttpHeaders.contentTypeHeader: "application/json",
...
class ClassroomGroupsBloc {
  ...
  Future<List<ClassroomGroup>> getData(String url) async{
    ...
    Iterable l = json.decode(response.body)['data'];
    List<ClassroomGroup> classroom = List<ClassroomGroup>.from(l.map((model)=> ClassroomGroup.fromJson(model)));
  ...
  Future<ClassroomGroup> getCurrentClassroomGroup(String id) async{
    ...
    var uri =
      Uri.https(BASE_URL, "/api/ClassroomGroups/"+id);
    final response = await http.get(uri, 
      headers: {
        HttpHeaders.authorizationHeader: "Bearer $token",
        HttpHeaders.contentTypeHeader: "application/json",
...
class BrbBloc{
  StreamController<List<BuildCount>> buildCountStreamController = StreamController<List<BuildCount>>.broadcast();
  Stream get getBuildCount => buildCountStreamController.stream;

  void update(List<BuildCount> b) {
    buildCountStreamController.sink.add(b); // add whatever data we want into the Sink
  }

  void dispose() {
    buildCountStreamController.close(); // close our StreamController to emory leak
  }
  User currentUser;
  ...
  void getClassroomGroupsBuildings(List<String> groupsIds) async{
    ...
    HashMap<int, int> buildings = new HashMap<int, int>();
      for (var id in groupsIds) {
          ClassroomGroup csg= await classroomGroupsBloc.getCurrentClassroomGroup(id);
      ...
      for (int item in buildings.keys) {
        ...
        counting.add(BuildCount(building: b, count: buildings[b.id]));
      }
...
class BuildCount{
  Building building;
  int count;
  BuildCount({this.building, this.count});
}
